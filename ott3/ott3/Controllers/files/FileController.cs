...
namespace ott3.Controllers.files
{
    ...
    public class FileController : ControllerBase
    {
        ...
        public async Task<IActionResult> Get(int id)
        {
            ...
            var stream = System.IO.File.OpenRead($"E:/dynamic/akash2/OTT/ott3/uploads/{id}");
            ...
            try
            {
                ...
                while ((section = await reader.ReadNextSectionAsync()) != null)
                {
                    ...
                    if (fileSection != null)
                    {
                        ...
                        var stream = new FileStream($"E:/dynamic/akash2/OTT/ott3/uploads/{p.uid}", FileMode.Create);

                        await section.Body.CopyToAsync(stream);
                        stream.Close();
                    }
                }
                p.url = $"https://localhost:7206/file/File/{p.uid}";
                p.status = "done";
                p.response = "ok";
                db.Files.Update(p);
                db.SaveChanges();
